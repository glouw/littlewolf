...
// Fast floor (math)
static inline int fl(const float x) { const int i = x; return i - (x < i); }

// Fast ceil (math)
static inline int cl(const float x) { const int i = x; return i + (x > i); }
...
static char tile(const Point a, char** const tiles)
{
    const int x = a.x;
    const int y = a.y;
    return tiles[y][x] - '0';
...
static inline float dec(const float x) { return x - (int) x; }
...
static Gpu setup(const int res)
{
    ...
    return (Gpu) { window, renderer, texture };
}
...
{
    const int height = focal(fov) * res / corrected.x + 0.5f;
    const int top = (res - height) / 2;
    const int bot = (res - top);
    return (Wall) { top < 0 ? 0 : top, bot > res ? res : bot };
}
...
static Hero spin(const Hero hero)
{
    ...
    SDL_PumpEvents();
    Hero step = hero;
    if(key[SDL_SCANCODE_H]) step.theta -= 0.1f;
    if(key[SDL_SCANCODE_L]) step.theta += 0.1f;
    return step;
}
...
static Hero move(const Hero hero, char** const walling)
{
    ...
    else step.velocity = mul(step.velocity, 1.0f - step.acceleration / step.speed);
    ...
    if(tile(step.where, walling)) step.velocity = (Point) { 0 }, step.where = hero.where;
    return step;
}
...
static inline uint32_t color(const char tile) { return 0xAA << (8 * (tile - 1)); }
